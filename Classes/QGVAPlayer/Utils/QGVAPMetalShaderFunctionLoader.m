...
- (void)loadDefaultLibraryIfNeed {
    ...
    NSBundle *bundle = [self qgvap_shaderResourceBundle];
...
- (NSBundle *)qgvap_shaderResourceBundle {
    
    NSArray<NSBundle *> *candidateBundles = @[[NSBundle bundleForClass:self.class], NSBundle.mainBundle];
    for (NSBundle *bundle in candidateBundles) {
        NSURL *bundleURL = [bundle URLForResource:@"QGVAPlayer" withExtension:@"bundle"];
        if (!bundleURL) {
            continue;
        }
