...
- (void)_decodeFrame:(NSInteger)frameIndex drop:(BOOL)dropFlag {
    ...
    if (!_mFormatDescription || !_mDecodeSession) {
        VAP_Error(kQGVAPModuleCommon, @"decompress fail! frame:%@ decoder session is unavailable", @(frameIndex));
        return;
    }
    ...
    _status = CMBlockBufferCreateWithMemoryBlock(kCFAllocatorDefault,
                                                 ...
                                                 packetData.length, 0,
                                                 &blockBuffer);    
    if (_status != noErr || !blockBuffer) {
        VAP_Error(kQGVAPModuleCommon, @"CMBlockBufferCreateWithMemoryBlock fail! frame:%@ error:%@", @(frameIndex), @(_status));
        return;
    }
    ...
    if (_status != noErr || !sampleBuffer) {
        VAP_Error(kQGVAPModuleCommon, @"CMSampleBufferCreateReady fail! frame:%@ error:%@", @(frameIndex), @(_status));
        return;
    }
    ...
    if (@available(iOS 9.0, *)) {
        ...
        NSData *retainedPacketData = packetData;
        ...
        if (status == kVTInvalidSessionErr) {
            ...
            if (status != noErr) {
                VAP_Error(kQGVAPModuleCommon, @"VTDecompressionSessionDecodeFrameWithOutputHandler fail! frame:%@ error:%@", @(frameIndex), @(status));
                CFRelease(sampleBuffer);
                return;
            }
        }
...
- (void)handleDecodePixelBuffer:(CVPixelBufferRef)pixelBuffer
    ...
    if (!pixelBuffer) {
        VAP_Error(kQGVAPModuleCommon, @"decompress fail! frame:%@ pixelBuffer is nil", @(frameIndex));
        return;
    }
...
- (BOOL)createDecompressionSession {
    ...
    CFDictionaryRef ioSurfaceProperties = CFDictionaryCreate(kCFAllocatorDefault,
                                                             NULL,
                                                             NULL,
                                                             0,
    ...
    CFNumberRef pixelFormat = CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &v);
    if (pixelFormat && ioSurfaceProperties) {
        const void *values[] = {
            pixelFormat,
            kCFBooleanTrue,
            kCFBooleanTrue,
